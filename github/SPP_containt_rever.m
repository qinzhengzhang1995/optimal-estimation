...
%%%%%%%%%
% for zzz = 1 : 13
%    hlin_nono = rir_generator(c, fs, r, s1(zzz,:), L, beta2, n, mtype);
%    hlin2_nono = hlin_nono';
%    h3_nono(:,:,zzz) = hlin2_nono;        %%%%%%%%%% h3 是最好的，可用于画图   h3_nono是没有（多径？）和混响的
% end

% for zzz = 1 : 13
%    hlin = rir_generator(c, fs, r, s1(zzz,:), L, beta, n, mtype);
%    hlin2 = hlin';
...
%%%%%%%%%数据
fileFolder = fullfile('F:\cssp\timit\signal\test_wav_16k'); %搜索目录
...
for fenpicun = 1 : 10
 for weizhi =  1 : 13
    ...
    for i = (fenpicun-1)*meizushuliang+1:fenpicun*meizushuliang
          ...
          count_i = count_i + 1
          %%
          xuanzeload = randi([0 4]);
%             xuanzeload = 2;
          if xuanzeload == 0
              load h3_020_newnew.mat
          elseif xuanzeload == 1
              load h3_040_newnew.mat
          elseif xuanzeload == 2
              load h3_060_newnew.mat
          elseif xuanzeload == 3
              load h3_030_newnew.mat
          elseif xuanzeload == 4
              load h3_050_newnew.mat
%           elseif xuanzeload == 5
%               load h3_070.mat
          end
          %%
...
name = ['testdata_SPP_containrever_new' num2str(fenpicun) '.h5']
h5create(name,'/x_test',[1032 t_separate numnum_cun]);
h5create(name,'/target_SPP',[1032 t_separate numnum_cun]);
% h5create('testdata_DRR_IRM_new1.h5','/y_test',[1 numnum_cun],'Datatype','int32');
% h5create(name,'/target_SPP',[1032 t_separate numnum_cun]);
% h5create(name,'/target_DRR',[1032 t_separate numnum_cun]);

h5write(name,'/x_test',x_LPS);
h5write(name,'/target_SPP',y_spp);
% h5write('testdata_DRR_IRM_new1.h5','/y_test',y_label);
...
%改beta、SNR、路径名称（test与test）、最后存的名字、h3是否load、分频。
