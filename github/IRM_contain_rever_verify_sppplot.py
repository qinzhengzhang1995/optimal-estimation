...
for xinzao in range(5, 15+1, 5):
    for hunxiang in range(60,61,20):
    # for hunxiang in range(20, 71, 10):
        for jiaodu in range(2,13,1):
        # for jiaodu in range(2, 13, 1):
            SPP_final= 10 * torch.ones(1032, 249, 250)
            ...
            for frequency in range(3):
                h5f_file = h5py.File('G:\\SPP_contain_rever\\verifydata_%ddb_0%d_noise92_100_05s_ang%d.h5' % (xinzao, hunxiang, jiaodu), 'r')
                ...
                model_name = os.path.join('SPP_containrever_highsame_noweightdecay_-5_%dfrequency_%depoch' % (frequency, 40))
            ...
            data_x0 = np.array(h5f_file['target_SPP'])
            data_x0 = data_x0.transpose(2, 1, 0)
            SPP_plot.spp_plot(data_x0[0:128,:,0],fs,jiequshijian)
            print('ddd')
            h5f_file_new = h5py.File('G:\\SPP_contain_rever\\spp5db_60.h5', 'r')
            data_x_new = np.array(h5f_file_new['spp'])
            data_x_new = data_x_new.transpose(1, 0)
            print('data_x0', data_x_new.shape)
            SPP_plot.spp_plot(data_x_new[:,: ], fs, jiequshijian)
            print('ccc')
            SPP_plot.spp_plot(spp_matrix[0:128,:,0], fs, jiequshijian)
            print('bbb')
            SPP_plot.spp_plot(spp_matrix[0:128,:,3], fs, jiequshijian)
            print('aaa')
            SPP_plot.spp_plot(spp_matrix[0:128, :, 2], fs, jiequshijian)
            print('aaa')
            SPP_plot.spp_plot(spp_matrix[0:128, :, 1], fs, jiequshijian)
            print('aaa')
            SPP_plot.spp_plot(spp_matrix[0:128, :, 4], fs, jiequshijian)
            print('aaa')
            SPP_plot.spp_plot(spp_matrix[0:128, :, 5], fs, jiequshijian)
            print('aaa')
            SPP_plot.spp_plot(spp_matrix[0:128, :, 6], fs, jiequshijian)
            print('aaa')
            SPP_plot.spp_plot(spp_matrix[0:128, :, 7], fs, jiequshijian)
            print('aaa')
            ...
            h5f = h5py.File('G:\\SPP_contain_rever\\result\\result_IRM_contain_rever_%ddb_0%d_ang%d.h5' % (xinzao, hunxiang, jiaodu), 'w')
            # h5f.create_dataset('result_SPP_tongdaofenkai_total', data=SPP_tongdaofenkai_total)
            h5f.create_dataset('result_SPP_tongdaobufenkai_total', data=SPP_final.cpu())
